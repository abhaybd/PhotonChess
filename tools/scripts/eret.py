"""
Eigenmann Rapid Engine Test (ERET) runner for Photon.

Evaluates the engine against the ERET suite over UCI. Parallelism is via
threads that each own a Photon subprocess — search runs in those engines, so
threads only wait on UCI I/O.

@see https://www.chessprogramming.org/Eigenmann_Rapid_Engine_Test
"""
...
class EPD:
    id: str
    fen: str
    best_moves: frozenset[chess.Move]
    avoid_moves: frozenset[chess.Move]
...
def _parse_info_stats(line: str) -> SearchStats | None:
    parts = line.split()
    ...
    while i < len(parts) - 1:
        ...
        if key == "depth":
            ...
            i += 1
    if depth is None or nodes is None or nps is None:
        return None
    return SearchStats(depth=depth, nodes=nodes, nps=nps)
    ...
    def search(self, fen: str, movetime_ms: int | None = None, depth: int | None = None) -> tuple[chess.Move, SearchStats]:
        ...
        while True:
            ...
            if line.startswith("info "):
                ...
                if stats is None:
                    raise RuntimeError("Engine returned bestmove without info stats")
                return chess.Move.from_uci(parts[1]), stats
...
def parse_epd(epd_str: str) -> EPD:
    ...
    ops = board.set_epd(epd_str.replace(",", " "))
    best = ops.get("bm", [])
    avoid = ops.get("am", [])
    return EPD(
        id=str(ops.get("id", "")),
        fen=board.fen(),
        best_moves=frozenset(best),
        avoid_moves=frozenset(avoid),
    )


def is_correct(epd: EPD, move: chess.Move) -> bool:
    if epd.best_moves:
        return move in epd.best_moves
    if epd.avoid_moves:
        return move not in epd.avoid_moves
    return False
...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Number of concurrent engine instances",
    )
    parser.add_argument(
        "-v",
        "--verbose",
...
def main() -> None:
    ...
    engines = [UCIEngine(engine_path) for _ in range(args.jobs)]
    idle: queue.SimpleQueue[UCIEngine] = queue.SimpleQueue()
    for engine in engines:
        idle.put(engine)

    def eval_position(epd_str: str) -> PositionResult:
        engine = idle.get()
        try:
        ...
        with ThreadPoolExecutor(max_workers=args.jobs) as executor:
            ...
            with tqdm(total=n, unit="pos") as pbar:
                for future in as_completed(futures):
                    ...
                    if result.correct:
                        correct += 1
                    total_depth += result.stats.depth
    ...
    print(
        f"Mean: depth={total_depth / n:.1f}, nodes={total_nodes / n:.0f}, "
        f"nps={total_nps / n:.0f}"
    )


# Eigenmann Rapid Engine Test positions.
# clang-format off
POSITIONS = [
