/**
 * Checks the engine against the Eigenmann Rapid Engine Test suite.
 *
...
/**
 * @brief Represents a position, plus additional metadata.
 *
...
struct epd_t {
	...
	std::optional<move_t> avoid_move;
};
...
epd_t ParseEPD(std::string_view epd_str) {
	std::stringstream ss;
	ss << epd_str;
	...
	fen_ss << "0 1";
	board_t board = MakeBoard(fen_ss);
	...
	while (ss) {
		...
		if (op == "id") {
			...
			epd.best_move = MoveFromShortNotation(board, value.substr(0, value.size() - 1));
		} else if (op == "am") {
			epd.avoid_move = MoveFromShortNotation(board, value.substr(0, value.size() - 1));
		}
	}
...
testresult_t run_test(const std::vector<epd_t>& epds, int depth) {
	...
	for (size_t i = 0; i < epds.size(); i++) {
		...
		if (epds[i].best_move && *epds[i].best_move == move) {
			correct++;
		} else if (epds[i].avoid_move && *epds[i].avoid_move != move) {
			correct++;
		}
	}
	auto end = std::chrono::steady_clock::now();
	return {correct, epds.size(), std::chrono::duration<float>(end - start).count()};
}
